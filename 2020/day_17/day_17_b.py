...
# now ALSO with w-dimension limits, w_min and w_max in the last two positions
pock_dim_limits = [0,0,0,0,0,0,0,0]
...
for row in map_0:
    col_num = col_num_ref
    for char in row:
        pock_dim[(col_num, row_num, 0, 0)] = char
...
def grow_dim_map(dim_map, dim_limits):
    ...
    w_min = int(dim_limits[6])
    w_max = int(dim_limits[7])
...
def count_neighbors(address, dim_map):
    ...
    for x_os in offsets:
        for y_os in offsets:
            for z_os in offsets:
                for w_os in offsets:
                    if (x_os, y_os, z_os, w_os) != (0,0,0,0):
                        nbr_address = (address[0] + x_os, address[1] + y_os, address[2] + z_os, address[3] + w_os)
                        try:
