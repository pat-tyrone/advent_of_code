...
for bag in bags:
    colors = []
    colors.append(bag.color)
    bags_to_open = bag.open_bag()
    
    while bags_to_open:
        current_bag = bags_to_open.pop(0)
        colors.append(current_bag.color)
        more_bags = current_bag.open_bag()
        for addl_bag in more_bags:
            bags_to_open.append(addl_bag)
    
    if 'shiny gold' in colors:
        bags_with_gold += 1

# total number of bags in which you could carry a shiny gold bag (subtracting 1 b/s question assumes
# that shiny gold bag will not be the outermost bag)
print(bags_with_gold -1)
